import { Component, OnInit } from '@angular/core';
import { UserService } from '../user.service';
import { Router } from '@angular/router';
import { Client } from '../model/client';
import { Agency } from '../model/agency';
import { Zahtev } from '../model/zahtev';

@Component({
  selector: 'app-admin',
  templateUrl: './admin.component.html',
  styleUrls: ['./admin.component.css']
})
export class AdminComponent implements OnInit {

  constructor(private userService: UserService, private router: Router) { }

  ngOnInit(): void {
    this.adminUsername = sessionStorage.getItem("username");
    this.adminPassword = sessionStorage.getItem("password");

    this.userService.getAllClients().subscribe((data: Client[])=>{
      this.allClients = data;
    })

    this.userService.getAllAgencies().subscribe((data: Agency[])=>{
      this.allAgencies = data;
    })

    this.userService.getAllClientsReq().subscribe((data: Client[])=>{
      this.allClientsReq = data;
    })

    this.userService.getAllAgenciesReq().subscribe((data: Agency[])=>{
      this.allAgenciesReq = data;
    })

    this.userService.getAllJobs().subscribe((data: Zahtev[])=>{
      this.allJobs = data;
    })

    this.uploadedImage = "data:image/png;base64,";
  }

  adminUsername: string;
  adminPassword: string;

  allJobs: Zahtev[] = [];
  allClients: Client[] = [];
  allAgencies: Agency[] = [];
  isEditing: Boolean = false;
  isEditingAgency: Boolean = false;
  usernameEditing: string;
  agencyEditing: string;
  addingClient: Boolean = false;
  addingAgency: Boolean = false;

  firstnameEdt: string;
  lastnameEdt: string;
  contactEdt: number;
  mailEdt: string;

  editClient(username: string){
    this.usernameEditing = username;
    this.isEditing = true;
  }

  updateClient(){
    this.userService.updateClient(this.usernameEditing, this.firstnameEdt, this.lastnameEdt, this.contactEdt, this.mailEdt).subscribe(resp=>{
      if(resp['message']!="Uspeh"){
        alert("Update failed!")
      }
    })

    this.ngOnInit();
    this.cancelEditing();
  }

  cancelEditing(){
    this.isEditing = false;
  }

  deleteClient(username: string) {
    this.userService.deleteClient(username).subscribe(resp=>{
      if(resp['message']!="Uspeh"){
        alert("Update failed!")
      }
    })

    this.ngOnInit();
  }

  username: string;
  password: string;
  password2: string;
  contact: string;
  mail: string;
  name: string;
  lastnameC: string;
  message: string;

  addClientForm(){
    this.addingClient = true;
  }
  cancelAddingClient() {
    this.addingClient = false;
    this.message2 = null;
    this.message = null;

    this.lastnameC = null;
    this.username = null;
    this.password = null;
    this.password2 = null;
    this.contact = null;
    this.mail = null;
    this.name = null;
  }

  addClient() {
    if(!(this.username && this.password && this.password2 && this.contact && this.mail && this.name && this.lastnameC)) {
      this.message = "Morate uneti sva polja."
      return;
    }
    if(this.isPasswordValid() == false) return;    

    if(this.password != this.password2) {
      this.message = "Ponovite lozinku!";
      return;
    }

    if(parseInt(this.contact)<=0 || isNaN(parseInt(this.contact, 10))) {
      this.message = "Kontakt telefon mora biti broj veci od 0!";
      return;
    }
    
    this.userService.registerKlijent(this.username, this.name, this.lastnameC, this.password, this.contact, this.mail, this.uploadedImage, "prihvacen").subscribe(respObj=>{
      if(respObj['message']=='ok'){
        this.ngOnInit();
        this.cancelAddingClient();
      }
      else if(respObj['message']=='notUnique'){
        this.message = "Korisnicko ime i mail moraju biti jedinstveni na nivou cele baze.";
      }
      else {
        this.message = "GRESKA"
      }
    });

  }

  isPasswordValid(): boolean {   //pomocna funkcija koja proverava ispravnost lozinke
    if(this.password.length<7 || this.password.length>12) {
      this.message2 = this.message = "Lozinka mora da sadrzi najmanje 7, a najvise 12 karaktera."
      return false;
    }

    const firstCaracter = this.password.charAt(0);
    if(!/[a-zA-Z]/.test(firstCaracter)) {
      this.message2 = this.message = "Prvi karakter lozinke mora biti slovo."
      return false;
    }

    if(!/[A-Z]/.test(this.password) || !/[0-9]/.test(this.password) || !/[^a-zA-Z0-9]/.test(this.password)) {
      this.message2 = this.message = "Lozinka mora sadrzati bar jedno veliko slovo, jedan broj i jedan specijalni karakter."
      return false;
    }

    return true;  //proslo je sve uslove
  }

  selectedImage: File;
  uploadedImage: string;

  onFileSelected(event: any) {    // poziva se kad korisnik izabere sliku za upload. Ovom metodom izvlacimo izabrani fajl iz dogadjaja i cuvamo ga u 'image' promenljivu
    this.selectedImage = event.target.files[0];
  }

  uploadImage(event: any) {
    event.preventDefault();
    if (!this.selectedImage) {
      return;
    }
  
    const reader = new FileReader();
    reader.onload = (e) => {
      const imageBase64 = e.target.result as string; // base64-encoded image data

      const image = new Image();
      image.onload = () => {
          const width = image.width;
          const height = image.height;

          if (width < 100 || height < 100 || width > 300 || height > 300) {
            alert("Slika mora imati minimalno dimenzije 100x100px i maksimalno dimenzije 300x300px.");
            return;
          }
          
          this.uploadedImage = imageBase64;
      };

      image.src = imageBase64;
    };

    reader.readAsDataURL(this.selectedImage);
  }

  //===============================================================

  agNameEdt: string;
  agContactEdt: number;
  agMailEdt: string;
  agAddrEdt: string;
  agDescrEdt: string;

  editAgency(username: string){
    this.agencyEditing = username;
    this.isEditingAgency = true;
  }

  cancelAgencyEditing(){
    this.isEditingAgency = false;
  }

  updateAgency(){
    this.userService.updateAgency(this.agencyEditing, this.agNameEdt, this.agContactEdt, this.agMailEdt, this.agAddrEdt, this.agDescrEdt).subscribe(resp=>{
      if(resp['message']!="Uspeh"){
        alert("Update failed!")
      }
    })

    this.ngOnInit();
    this.cancelAgencyEditing();
  }

  deleteAgency(username: string) {
    this.userService.deleteAgency(username).subscribe(resp=>{
      if(resp['message']!="Uspeh"){
        alert("Update failed!")
      }
    })

    this.ngOnInit();
  }

  addressA: string;
  numberA: string;
  descrA: string;
  message2: string;

  addAgencyForm(){
    this.addingAgency = true;
  }
  cancelAddingAgency() {
    this.addingAgency = false;    
    this.message2 = null;
    this.message = null;

    this.addressA = null;
    this.numberA = null;
    this.descrA = null;
    this.username = null;
    this.password = null;
    this.password2 = null;
    this.contact = null;
    this.mail = null;
    this.name = null;
  }

  addAgency() {
    if(!(this.username && this.password && this.password2 && this.contact && this.mail && this.name && this.addressA && this.numberA, this.descrA)) {
      this.message2 = "Morate uneti sva polja."
      return;
    }
    if(this.isPasswordValid() == false) return;    

    if(this.password != this.password2) {
      this.message2 = "Ponovite lozinku!";
      return;
    }

    if(parseInt(this.contact)<=0 || isNaN(parseInt(this.contact, 10))) {
      this.message2 = "Kontakt telefon mora biti broj veci od 0!";
      return;
    }

    if(parseInt(this.numberA)<=0 || isNaN(parseInt(this.numberA, 10)) ) {
      this.message2 = "Maticni broj agencije mora biti broj veci od 0!";
      return;
    }
    
    this.userService.registerAgencija(this.username, this.password, this.contact, this.mail, this.name, this.addressA, this.numberA, this.descrA, this.uploadedImage, "prihvacen").subscribe(respObj=>{
      if(respObj['message']=='ok'){
        this.ngOnInit();
        this.cancelAddingAgency();
      }
      else if(respObj['message']=='notUnique'){
        this.message2 = "Korisnicko ime i mail moraju biti jedinstveni na nivou cele baze.";
      }
      else {
        this.message2 = "GRESKA"
      }
    });

  }

  // ==================================== PROMENA LOZINKE =======================================
  oldPassword: string;
  newPassword: string;
  repeatPassword: string;
  message3: string;

  changePassword() {
    console.log("promena");
    if(!( this.oldPassword && this.newPassword && this.repeatPassword )) {
      this.message3 = "Morate uneti sva polja."
      return;
    }
    if(this.canChangePassword() == false) return;    

    if(this.oldPassword != this.adminPassword) {
      this.message3 = "Stara lozinka nije ispravna.";
      return;
    }

    if(this.newPassword != this.repeatPassword) {
      this.message3 = "Ponovljena lozinka mora biti ista kao nova.";
      return;
    }

    this.userService.changePassword("admin", this.adminUsername, this.newPassword).subscribe(resp=>{    //subscribe - cekam odgovor u userFromDB
      if(resp){
        alert(resp['message']);
        // obrisi sva polja i message
        this.oldPassword = this.newPassword = this.repeatPassword = this.message3 = null;
      }
      else {
        this.message3 = "Promena lozinke neuspesna."
      }
    })

  }

  canChangePassword(): boolean {   //pomocna funkcija koja proverava ispravnost lozinke
    if(this.newPassword.length<7 || this.newPassword.length>12) {
      this.message3 = "Lozinka mora da sadrzi najmanje 7, a najvise 12 karaktera."
      return false;
    }

    const firstCaracter = this.newPassword.charAt(0);
    if(!/[a-zA-Z]/.test(firstCaracter)) {
      this.message3 = "Prvi karakter lozinke mora biti slovo."
      return false;
    }

    if(!/[A-Z]/.test(this.newPassword) || !/[0-9]/.test(this.newPassword) || !/[^a-zA-Z0-9]/.test(this.newPassword)) {
      this.message3 = "Lozinka mora sadrzati bar jedno veliko slovo, jedan broj i jedan specijalni karakter."
      return false;
    }

    return true;  //proslo je sve uslove
  }


  logout(){
    sessionStorage.clear();
    this.router.navigate(['']);
  }

  // =========================================== ZAHTEVI ZA REG ===================================
  allClientsReq: Client[] = [];
  allAgenciesReq: Agency[] = [];

  prihvatiReg(username: string, type: string){
    if(type == "client"){
      this.userService.acceptClient(username, "prihvacen").subscribe(resp=>{
        if(resp['message']=="ok"){          
          this.ngOnInit();
        }
      })
  
    }
    else if(type == "agency"){
      this.userService.acceptAgency(username, "prihvacen").subscribe(resp=>{
        if(resp['message']=="ok"){   
          this.ngOnInit();
        }
      })
    }
  }

  odbijReg(username: string, type: string){
    if(type == "client"){
      this.userService.acceptClient(username, "odbijen").subscribe(resp=>{
        if(resp['message']=="ok"){          
          this.ngOnInit();
        }
      })
  
    }
    else if(type == "agency"){
      this.userService.acceptAgency(username, "odbijen").subscribe(resp=>{
        if(resp['message']=="ok"){   
          this.ngOnInit();
        }
      })
    }
  }

  // ================================= RADNA MESTA ===========================
  radnaMesta: number;
  message4: string;

  odobriRadnaMesta(agencijaUsername: string, radnihMesta: number, radnaMestaZahtev: number){
    if(this.radnaMesta<=0 || this.radnaMesta == null){
      this.message4 = "Upisite 1 ili vise radnih mesta."
      return
    }
    if(this.radnaMesta > radnaMestaZahtev){
      this.radnaMesta = radnaMestaZahtev;
    }

    this.userService.odobriRadnaMesta(agencijaUsername, this.radnaMesta + radnihMesta).subscribe(resp=>{      
      if(resp){   
        console.log(this.radnaMesta + radnihMesta);
        this.message4 = this.radnaMesta = null;
        this.ngOnInit();
      }
    })

  }

  

}
